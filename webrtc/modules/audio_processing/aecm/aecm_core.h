...
#include "webrtc/modules/audio_processing/smd/smd.h"
#endif
...
#ifdef AFC
typedef struct{
	uint16_t veryLongTermSpecHis[VERY_LONG_TERM_BLOCK_NUM][PART_LEN1];
	uint16_t veryLongTermPeakPosHis[VERY_LONG_TERM_BLOCK_NUM][PEAK_NUM];	
	uint16_t veryLongTermPos;	
	int16_t howlingPos;
	int16_t candidatePos;
	int16_t longTermFrameNum;
	float howlingScaleFactorSmoothed[PART_LEN1];
	float howlingPossibility; 
	float afcInOutEnRatioSmoothed;	
}AudioFeedbackCancellation;
...
typedef struct {
	...
	float nearFarComplexCorr[PART_LEN1];
	float errorFarCorr[PART_LEN1];
	float nearPow[PART_LEN1];
	float farPow[PART_LEN1];
	ComplexFloat nearFarPow[PART_LEN1];
	int16_t farMainBandFlag[PART_LEN1];
	uint16_t farSortedValue[PART_LEN1];
	uint16_t farSmoothedValue[PART_LEN1];
	...
	uint32_t* nearFarBitCountCorrHis;
	uint32_t nearFarBitCountCorrHisBuf[MAX_BUF_LEN+16];	


	int16_t cutHighFrequencyEnable;
    ...
    // Pointers to the above buffers
	 ...
	 int16_t channelStoredBackup[PART_LEN1];	
	int16_t echoStoredBackupLogEnergy[MAX_BUF_LEN]; 
	int16_t backupFilterNeverUpdate;
    int32_t mseStoredBackUpOld;
	int32_t diffStoredBackupSmoothed;
	int32_t mseStoredSmoothed;
	int32_t mseAdaptSmoothed;
    int16_t echoFarEnRatioFromBackUp;
    int32_t mseStoredBand[PART_LEN1];
    float preFilter[PART_LEN1];
    float preFilterInOutEnRatio;
    int16_t mseStoredSmallerCnt;
    uint32_t backupFilterUpdateTime;
    int32_t minMseStored;
    int16_t storedFilterCandidate[PART_LEN1];
    uint32_t diffStoredCandidate;
    int32_t mseReach20Cnt;
    int32_t backupFilterUpdateCnt;
    int16_t mseChannelCntClearCnt;
    


	int16_t channelAdaptCorr;
	int16_t farPreNowFrameCorr;
	int16_t corrDtFlag;	 
	int16_t channelAdaptOld16[PART_LEN1];
    int32_t channelAdaptOld32[PART_LEN1];
	...
	int16_t  AecmMode;//add by wuzhaoyang
	int16_t  AecmModeArray[1024];
	int16_t  fromReceiverToSpeaker;
	int16_t  fromSpeakerToReceiver;
    float speakerGain;
	...
	//struct afc
	int16_t afcEnable;//added by chgx 20170220
#ifdef AFC
	AudioFeedbackCancellation afcInstance;
	//end afc
#endif
    ...
    FILE *farFile;
	FILE *farFile_block;
	FILE *farFile_aligned;
    FILE *nearFile;
	FILE *nearFile_1;
    FILE *outFile;
	FILE *speakerModeFile;	
    FILE *infoFile;
	FILE*jitterFile;
#endif

   int32_t nearEnergyHis[MAX_BUF_LEN];  
   int32_t farEnergyHis[MAX_BUF_LEN]; 
   int16_t nearFarCorr;   
   uint16_t nearSpecHis[SPEC_BUF_LEN*PART_LEN1];
   uint16_t farSpecHis[SPEC_BUF_LEN*PART_LEN1];   
   uint16_t *nearSpecPtr; 
   int16_t nearFarEnRatioHis[SPEC_BUF_LEN];
   int32_t echoFarEnRatioNeed;
   int32_t echoFarEnRatio;
   int16_t echoFarEnRatioValidFlag;
   int16_t echoFarEnRatioUpdateFlag;    
   int32_t effectOutEnAvg;
   int32_t currentResidualEnAvg;
   int32_t nearEnavg;
   int32_t farEndScaleFactor; 
   int32_t farEndScaleFactorSpeakerOn;
   int32_t farEndScaleFactorSpeakerOff; 




   int32_t stream_has_echo;
   int32_t farFrameDropCnt;
...
#endif
#ifdef MUSIC_OR_SPEECH
	int16_t nearIsSpeech;
	smd_h farSmdInst;
	smd_h nearSmdInst;
	float farMusicProb[500];//2s
	float nearMusicProb[500];//2s
	int16_t farSmdAudioBuf[SMD_BUF_MAX_SIZE];//max sampleRate is 32000,this buf can store up to 40ms audio samples
	int16_t nearSmdAudioBuf[SMD_BUF_MAX_SIZE];//max sampleRate is 32000,this buf can store up to 40ms audio samples
	int16_t smdBufLeft;
    int16_t far_history_time[PART_LEN * MAX_DELAY];
    int16_t residualBuf[PART_LEN];
	float *nearBandTonality;
	float *farBandTonality;
#endif
#ifdef SUPPORT_32K
    int32_t avgHnlInLowBand16k[3];
    int16_t HnlInLowBand16k[3][PART_LEN1];
    int16_t processBlockCnt;   
#endif
} AecmCore;
...
#ifdef MUSIC_OR_SPEECH
const int16_t* WebRtcAecm_AlignedFarendTime(AecmCore* self,  int delay);
#endif
