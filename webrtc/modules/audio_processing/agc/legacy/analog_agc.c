...
/////////////////////////modified by king on 2015-9-16//////////////////////////////////////
/* Gain/Suppression tables for virtual Mic (in Q10) 
 * Matlab code
 * kGainTableVirtualMic = (40.^(linspace(0,10,128)/20) * 2^10); */

static const uint16_t kGainTableVirtualMic[128] = {1024, 1039, 1054, 1070, 1085, 1101, 1117, 1134,
1150, 1167, 1184, 1201, 1219, 1237, 1255, 1273,1292, 1311, 1330, 1349, 1369, 1389, 1409, 1430,
1451, 1472, 1494, 1516, 1538, 1560, 1583, 1606,1630, 1654, 1678, 1702, 1727, 1753, 1778, 1804,
1831, 1857, 1885, 1912, 1940, 1968, 1997, 2026,2056, 2086, 2117, 2148, 2179, 2211, 2243, 2276,
2309, 2343, 2378, 2412, 2448, 2483, 2520, 2557,2594, 2632, 2670, 2710, 2749, 2789, 2830, 2872,
2914, 2956, 2999, 3043, 3088, 3133, 3179, 3225,3273, 3320, 3369, 3418, 3468, 3519, 3571, 3623,
3676, 3730, 3784, 3839, 3896, 3953, 4010, 4069,4129, 4189, 4250, 4312, 4376, 4440, 4505, 4570,
4637, 4705, 4774, 4844, 4915, 4987, 5059, 5133,5209, 5285, 5362, 5441, 5520, 5601, 5683, 5766,
5850, 5936, 6023, 6111, 6200, 6291, 6383, 6476};
                         ...
                         int32_t *micLevelOut)
{
    ...
    const int16_t kZeroCrossingLowLim = 20;    // modified by king on 2015-9-15
    const int16_t kZeroCrossingHighLim = 40;    // modified by king on 2015-9-15
	const int16_t virtualMicClampValue = 16384;
    ...
    if ((frameNrg < 1000) || (numZeroCrossing <= 10))   // modified by king on 2015-9-13
    {
	...
	//gainIdx = 127;//chgx assign gainIndex to a constant value 20130329
    ...
    for (ii = 0; ii < samples; ii++)
    {
		...
		if (tmpFlt > virtualMicClampValue)//modify by wuzhaoyang20160413  32767
        {
			tmpFlt = virtualMicClampValue;
            gainIdx--;
            if (gainIdx >= 127)
            {
		...
		if (tmpFlt < -virtualMicClampValue)
        {
			tmpFlt = -virtualMicClampValue;
            gainIdx--;
            if (gainIdx >= 127)
...
int WebRtcAgc_Process_oneTime(void *agcInst, const int16_t* const* in_near,
	...
	uint8_t *saturationWarning)
{
	...
	int loopTime = 1; 
	int retValue = 0;
	int16_t**inNearTmp = (int16_t**)in_near;
	if (stt->usedConfig.compressionGaindB > 15)
	{
		loopTime = 15 + (stt->usedConfig.compressionGaindB - 15) / 3;
		loopTime = 1 + (loopTime - 15) / 25;
	}
	if (loopTime > 2)
	{
		loopTime = 2;
	...
	do
	{
		retValue = WebRtcAgc_Process_oneTime(agcInst, 
                                        (const int16_t* const*)inNearTmp,
                                        num_bands,
                                        samples, 
                                        out,
                                        inMicLevel,
		...
		if (loopTime > 0)
		{
			...
			{
				memcpy(inNearTmp[i], out[i], samples*sizeof(int16_t));
			}
		}
	} while (loopTime>0);
	return retValue;
...
int WebRtcAgc_set_config(void* agcInst, WebRtcAgcConfig agcConfig) {
  ...
  stt = (LegacyAgc*)agcInst;
	...
	//chgx added begin
	if (agcConfig.compressionGaindB > 30)
	{
		agcConfig.compressionGaindB = 30;
	}
	if ((agcConfig.compressionGaindB > 15) && (agcConfig.compressionGaindB <= 30))
	{
		agcConfig.compressionGaindB = 15+(agcConfig.compressionGaindB-15)*5;
	}
	//chgx added end
...
int WebRtcAgc_get_config(void* agcInst, WebRtcAgcConfig* config) {
  ...
  stt = (LegacyAgc*)agcInst;
	...
	//chgx added begin
	if (config->compressionGaindB > 15)
	{
		config->compressionGaindB = 15 + (config->compressionGaindB - 15) / 5;
	}
	//chgx added end
...
#ifdef WEBRTC_AGC_DEBUG_DUMP
#ifdef ANDROID
    char *fpt = "/mnt/sdcard/agc_test_log.txt";
    char *agcLog = "/mnt/sdcard/agc_debug_log.txt";
    char *logFile = "/mnt/sdcard/agc_digit_log.txt";
#else
    char *fpt = "./agc_test_log.txt";
    char *agcLog = "./agc_debug_log.txt";
    char *logFile = "./agc_digit_log.txt";
#endif
    stt->fpt = fopen(fpt, "wt");
    stt->agcLog = fopen(agcLog, "wt");
    stt->digitalAgc.logFile = fopen(logFile, "wt");
#endif
