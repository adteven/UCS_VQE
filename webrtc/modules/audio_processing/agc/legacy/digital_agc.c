...
int32_t WebRtcAgc_ProcessDigital(DigitalAgc* stt,
    ...
    {
        if (stt->vadNearend.stdLongTerm < 2000)
        {
            //add by king 

			// decay = (int16_t)(((stt->vadNearend.stdLongTerm - 2000) * (decay>>4)) >> 12);
			tmp32 = WEBRTC_SPL_MUL_16_16((stt->vadNearend.stdLongTerm ), WEBRTC_SPL_RSHIFT_W32(decay, 4));
			decay = (int16_t)WEBRTC_SPL_RSHIFT_W32(tmp32, 12);
			//decay = 0;
        }
        //add by king ,on 20150727
		else if (stt->vadNearend.stdLongTerm < 4000)
		{
			// decay = (int16_t)(((stt->vadNearend.stdLongTerm - 2000) * decay) >> 12);
			tmp32 = WEBRTC_SPL_MUL_16_16((stt->vadNearend.stdLongTerm - 2000), decay);
			decay = (int16_t)WEBRTC_SPL_RSHIFT_W32(tmp32, 12);
		} 
