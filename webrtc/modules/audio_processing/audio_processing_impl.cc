...
#ifdef APM_RECORD_DEBUG
  memset(pathname_, 0x00, sizeof(pathname_));
  bfhighpass = NULL;
  bfvirtualmic = NULL;
  bfaec = NULL;
  bfaecm = NULL;
...
#endif
#ifdef APM_RECORD_DEBUG
	//add by wuzhaoyang20160407	
	if (bfhighpass != NULL)
	{
		fclose(bfhighpass);
		bfhighpass = NULL;
	}
	if (bfvirtualmic != NULL)
	{
		fclose(bfvirtualmic);
		bfvirtualmic = NULL;
	}
...
#ifdef AECM_MAX_SAMPLE //high sample rate aecm enable
	  if (min_proc_rate > AECM_MAX_SAMPLE)
	  {
		  fwd_proc_rate = AECM_MAX_SAMPLE;
	  }
#else
	  fwd_proc_rate = kSampleRate16kHz;
#endif
...
int AudioProcessingImpl::ProcessStream(AudioFrame* frame) {
  ...
  if (echo_control_mobile_->is_enabled() &&
      frame->sample_rate_hz_ > kSampleRate16kHz) {
#ifdef AECM_MAX_SAMPLE //high sample rate aecm enable
	  if (frame->sample_rate_hz_ > AECM_MAX_SAMPLE)
	  {
		  LOG(LS_ERROR) << "the max supports sample rates in AECM is" << AECM_MAX_SAMPLE;
	  }
#else
...
int AudioProcessingImpl::ProcessStreamLocked() {
    ...
    if (bfhighpass == NULL)
    {
        sprintf(filename, "%s%s", pathname_, "bfhighpass.pcm");
        bfhighpass = fopen(filename, "wb");
    }
    memset(filename, 0x00, sizeof(filename));
    if (bfvirtualmic == NULL)
    {
        sprintf(filename, "%s%s", pathname_, "bfvirtualmic.pcm");
        bfvirtualmic = fopen(filename, "wb");
    }
    memset(filename, 0x00, sizeof(filename));
    if (bfaec == NULL)
    {
        sprintf(filename, "%s%s", pathname_, "bfaec.pcm");
        bfaec = fopen(filename, "wb");
    }
    memset(filename, 0x00, sizeof(filename));
    if (bfaecm == NULL)
    {
        sprintf(filename, "%s%s", pathname_, "bfaecm.pcm");
        bfaecm = fopen(filename, "wb");
    }
    memset(filename, 0x00, sizeof(filename));
    if (bfnsx == NULL)
    {
        sprintf(filename, "%s%s", pathname_, "bfnsx.pcm");
        bfnsx = fopen(filename, "wb");
    }
    memset(filename, 0x00, sizeof(filename));
    if (bfagc == NULL)
    {
        sprintf(filename, "%s%s", pathname_, "bfagc.pcm");
        bfagc = fopen(filename, "wb");
    }
    memset(filename, 0x00, sizeof(filename));
    if (afagc == NULL)
    {
        sprintf(filename, "%s%s", pathname_, "afagc.pcm");
        afagc = fopen(filename, "wb");
    }
#endif
	...
	if (bfhighpass)
	{
		for (int i = 0; i < ca->num_channels(); i++)
		{
			fwrite(ca->split_bands(i)[kBand0To8kHz], 2, ca->num_frames_per_band(), bfhighpass);
		}
	}
#endif
	...
	if (bfvirtualmic)
	{
		for (int i = 0; i < ca->num_channels(); i++)
		{
			fwrite(ca->split_bands(i)[kBand0To8kHz], 2, ca->num_frames_per_band(), bfvirtualmic);
		}
	}
#endif
...
#ifdef APM_RECORD_DEBUG
	if (bfaec && echo_cancellation_->is_enabled())
	{
		for (int i = 0; i < ca->num_channels(); i++)
		{
			fwrite(ca->split_bands(i)[kBand0To8kHz], 2, ca->num_frames_per_band(), bfaec);
		}
	}
#endif
	...
	// RETURN_ON_ERR(noise_suppression_->ProcessCaptureAudio(ca));//skipped by chgx 20160302	
#ifdef APM_RECORD_DEBUG
    if (bfaecm && echo_control_mobile_->is_enabled())
	{
		for (int i = 0; i < ca->num_channels(); i++) 
		{
			int16_t* noisy = (short *)ca->low_pass_reference(i);
			int16_t* clean = (short *)ca->split_bands_const(i)[kBand0To8kHz];
			if (noisy == NULL)
			{
				noisy = clean;
				clean = NULL;
			}
			fwrite(noisy, 2, ca->num_frames_per_band(), bfaecm);
		}
    }
#endif
  ...
  if (bfnsx)
  {
	  for (int i = 0; i < ca->num_channels(); i++)
	  {
		  fwrite(ca->split_bands(i)[kBand0To8kHz], 2, ca->num_frames_per_band(), bfnsx);
	  }
  }  
#endif
  RETURN_ON_ERR(noise_suppression_->ProcessCaptureAudio(ca));//added by chgx 20160302
#ifdef APM_RECORD_DEBUG
  if (bfagc)
  {
	  for (int i = 0; i < ca->num_channels(); i++)
	  {
		  fwrite(ca->split_bands(i)[kBand0To8kHz], 2, ca->num_frames_per_band(), bfagc);
	  }
  }
#endif
  ...
  if (afagc)
  {
	  for (int i = 0; i < ca->num_channels(); i++)
	  {
		  fwrite(ca->split_bands(i)[kBand0To8kHz], 2, ca->num_frames_per_band(), afagc);
	  }
  }
#endif
...
int AudioProcessingImpl::GetReverseStreamLockedScaleFactor()
{
	return echo_control_mobile_->GetScaleFactor();
}
...
int AudioProcessingImpl::set_filepath(char *filepath)//add by wuzhaoyang20160407
{
	int retval = -1;
...
void AudioProcessingImpl::SetPcmRecordPath(const char* pathname)
{
#ifdef APM_RECORD_DEBUG
    memcpy(pathname_, pathname, sizeof(pathname_));
#endif
}
