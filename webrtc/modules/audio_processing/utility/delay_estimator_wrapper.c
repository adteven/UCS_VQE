...
#include"webrtc/modules/audio_processing/aecm/aecm_defines.h"//added by chgx

// Only bit |kBandFirst| through bit |kBandLast| are processed and
// |kBandFirst| - |kBandLast| must be < 32.
#ifdef CHANGE_BINARY_BAND
enum { kBandFirst = 3 };
enum { kBandLast = 34 };
#else
enum { kBandFirst = 12 };
...
void WebRtc_FreeDelayEstimator(void* handle) {
  ...
  free(self->mean_out_spectrum);//chgx
  self->mean_out_spectrum = NULL;
...
void* WebRtc_CreateDelayEstimator(void* farend_handle, int max_lookahead) {
  ...
  if (self != NULL) {
	...
	self->mean_out_spectrum = NULL;  //chgx
    ...
    // Allocate memory for spectrum buffers.
	self->mean_out_spectrum = malloc(farend->spectrum_size * sizeof(SpectrumType));
	memory_fail |= (self->mean_out_spectrum == NULL);
...
int WebRtc_InitDelayEstimator(void* handle) {
  ...
  // Set averaged far and near end spectra to zero.
  memset(self->mean_out_spectrum, 0,
	  sizeof(SpectrumType) * self->spectrum_size);//chgx
  self->out_spectrum_initialized = 0;//chgx
...
int WebRtc_DelayEstimatorProcessFix(void* handle,
  ...
  binary_spectrum = BinarySpectrumFix(near_spectrum,
    ...
    self->near_binary_spectrum = binary_spectrum;
...
//USE_BINARY_SPECTRUM_BEFORE_IFFT_OUT
uint32_t WebRtc_Process_Out_Binary_Spectrum(void* handle, uint16_t* out_spectrum,int out_q)
{
	DelayEstimator* self = (DelayEstimator*)handle;
	uint32_t binary_out_spectrum = 0;	

	// Get binary spectra.
	binary_out_spectrum = BinarySpectrumFix(out_spectrum,self->mean_out_spectrum,out_q,&(self->out_spectrum_initialized));
	return binary_out_spectrum;
