...
int32_t TraceWindows::AddTime(char* trace_message,
  ...
  if (level == kTraceApiCall) {
    ...
    sprintf(trace_message, "(%02u:%02u:%02u:%3u |%5u) ", system_time.wHour,
...
int32_t TraceWindows::AddDateTimeInfo(char* trace_message) const {
  ...
  // sprintf() cann't convert TCHAR to char string
#if 0
  ...
  sprintf(trace_message, "Local Date: %ls Local Time: %ls", sz_date_str,
          sz_time_str);
#else
  sprintf(trace_message, "Local Date: %d-%02d-%02d Local Time: %02d:%02d:%02d", \
